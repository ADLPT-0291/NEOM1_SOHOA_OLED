...
def has_ipv4_address(interface):
    try:
        ...
        print(f"loi kiem tra mang Eth0: {e}")
        return False
    ...
    try:
        ...
        if '+CSQ' in response:
            try:
                ...
                print(f"Error processing RSSI value: {e}")
                return None
        else:
    ...
    try:
        ...
        if '+COPS' in response:
            try:
                ...
                print(f"Error processing operator info: {e}")
                return "Unknown"
        else:
            return "Unknown"
    except Exception as e:
...
def job_GuiApiPhatLaiLichPhatKhanCap(LichPhat):
    try:
        ...
        if trave['data'] == 'true':
            ...
            print(f'Loi Gui lich Phat lai Lich phat: {e}')
    ...
    try:
      ...
      if Ban_Tin['kieunguon'] == "Tiếp Sóng":
        ...
        if(phatbantintinh == False and PhatKhanCap == False):
            ...
            print(f'loi chay job phat ban tin noi bo theo lich "job_PhatBanTinNoiBo": {e}')
...
def kiem_tra_lich_phat_dang_phat(LichPhat, NguonPhats):
    ...
    if LichPhatDangNhan == {}:
        ...
        try:
            ...
            print(f'loi kiem tra phat ban tin noi bo: {e}')
...
def api_xacnhanketnoi(data):
    ...
    try:
        ...
        print(f'loi xac nhan ket noi: {e}')
  # get_speedtest()
...
def pingTinh():
  ...
  if trangthaiguiApi:
      try:
        ...
        print(f'loi call api ping tinh: {e}')
...
def pingServer():
    ...
    try:
        ...
        # if not run_main:
        #   nhapnhatLedConnectCallApiloi.stop()
        print(f'loi ping 20s ve server: {e}')
...
def playBantinTinh(data, status):
  ...
  try:
    ...
    print(f"Loi phat ban tin tinh: {e}")
...
def playBantinTinh2(data):
  ...
  try:
    ...
    print(f"loi ping ban tin dang phat play ve server: {e}")
